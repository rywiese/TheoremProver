...
let s = (ForAll ("x", Implies (ForAll ("y", Implies (Equals (Times (Const (Int 2), Var "y"), Const (Int 1)), Equals (Plus (Var "x", Var "y"), Const (Int 1)))), Exists ("y", Equals (Plus (Var
    "y", Var "x"), Const (Int 1)))))) in expandCNF (cnf s)
    in insertAndReplace ( (cnf s)) [(Const (Skol ("a",["x"])))]
...
let s2 = Not (ForAll ("y", Equals (Plus(Const (Int 3), Var "y"),Plus(Const (Int 3), Var "y")))) in unifyStmt (cnf s1) (cnf s2)
resolve (cnf s2) (cnf s1)
